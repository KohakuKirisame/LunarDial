...
# -------------------------------------------------------
# @By:Kohaku Cao
# @Time:2022-06-29 19:07
# @QQ:1848790911
# @Mail:kohaku@hepnovel.com
# @File:ExtendedClasses.py
# @For:月時計 ~Lunar Dial~
# -------------------------------------------------------
...
class dayLabel(QLabel):
    ...
    def mouseDoubleClickEvent(self,event):
        if self.date!=(-1,-1,-1):
            Home.home.toDayView(self.date)

    def isToday(self):
        date="%04d-%02d-%02d" %(self.date[0],self.date[1],self.date[2])
        if date==time.strftime("%Y-%m-%d",time.localtime()):
            self.setStyleSheet("background-color:#458bff;")
        else:
            self.setStyleSheet("background-color:none;")

class weatherView(QWebEngineView):
    def __init__(self,parent):
        super().__init__(parent=parent)
        self.page().setBackgroundColor(Qt.transparent)

class bgImage(QLabel):
    def __init__(self,parent):
        super().__init__(parent=parent)
        self.op = QGraphicsOpacityEffect()
        self.op.setOpacity(0.16)
