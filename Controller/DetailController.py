...
class DetailController(QWidget):
    def __init__(self,id,isAlarm=False):
        ...
        if self.isAlarm:
            ...
            self.effect.setSource(QUrl.fromLocalFile("Resources/alarm.wav"))
            self.effect.play()
    ...
    def setupContent(self, id):
        ...
        self.ui.Time2.setText(time.strftime("%Y-%m-%d %X",time.localtime(rtime)))
        self.ui.Remind2.setText(time.strftime("%Y-%m-%d %X",time.localtime(remind)))
        self.ui.Content2.setText(content)
        return 0
